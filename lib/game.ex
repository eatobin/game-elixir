defmodule Game do
  ...
  def start_game do
    ...
    game_state = %GameState{board: [0,1,2,3,4,5,6,7,8], player: turn_symbol, depth: 0, human_symbol: human_symbol, ai_symbol: ai_symbol}
  ...
  def play_moves(game_state) do
    ...
    display_board = List.replace_at(new_board, move, [IO.ANSI.red, game_state.player, IO.ANSI.reset])
    ...
    cond do
      GameState.won?(%{game_state | board: new_board}) ->
        IO.write("\nThe game is over and " <> IO.ANSI.red <> "<<< #{game_state.player} >>>" <> IO.ANSI.reset <> " won! Thanks for playing.\n\n")
      GameState.is_gameover?(%{game_state | board: new_board}) ->
        IO.write("\nThe game is over and " <> IO.ANSI.red <> "<<< it's a tie! >>>" <> IO.ANSI.reset <> " Thanks for playing.\n\n")
      true -> play_moves(%{game_state | board: new_board, player: GameState.toggle_player(game_state)})
    end
  end
